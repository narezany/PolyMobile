...
public sealed partial class LuauProvider : IScriptLanguageProvider
{
	...
	private static readonly HashSet<string> MobileSafeGlobalData =
	[
		"Vector3",
		"Vector2",
		"Quaternion",
	...
	public void Run(Script script)
	{
		...
		CrashReporter.SetBreadcrumb("scriptRun", "luau-init-start " + script.NetworkPath);
		LuaState state = InitalizeScript(script);
		CrashReporter.SetBreadcrumb("scriptRun", "luau-init-done " + script.NetworkPath);

		// Try compile
		bool useMobileSourceLoad = (Globals.IsMobileBuild || OS.HasFeature("mobile-ui")) && !string.IsNullOrEmpty(script.Source);
		if (useMobileSourceLoad)
		{
			CrashReporter.SetBreadcrumb("scriptRun", "compile-skip-mobile-source " + script.NetworkPath);
		}
		else try
		{
			CrashReporter.SetBreadcrumb("scriptRun", "compile-start " + script.NetworkPath);
			script.TryCompile();
			CrashReporter.SetBreadcrumb("scriptRun", "compile-done " + script.NetworkPath);
		}
		catch (Exception e)
		{
		...
		try
		{
			CrashReporter.SetBreadcrumb("scriptRun", "load-start " + script.NetworkPath);
			if (useMobileSourceLoad)
			{
				state.LoadString(script.LuaPath, script.Source);
			}
			else
			{
				state.Load(script.LuaPath, script.Bytecode!);
			}
			CrashReporter.SetBreadcrumb("scriptRun", "load-done " + script.NetworkPath);

			async void run()
			{
				CrashReporter.SetBreadcrumb("scriptRun", "resume-start " + script.NetworkPath);
				await ResumeThread(state, null, 0, isMainThread: true);
				CrashReporter.SetBreadcrumb("scriptRun", "resume-done " + script.NetworkPath);
			}
	...
	{
		bool mobileLuaSafeMode = Globals.IsMobileBuild || OS.HasFeature("mobile-ui");

		CrashReporter.SetBreadcrumb("luauInit", (mobileLuaSafeMode ? "new-state-mobile " : "new-thread-global ") + script.NetworkPath);
		LuaState state = mobileLuaSafeMode ? CreateMobileScriptState() : NewThread(GlobalLuaState);
		script.LuauState = state;

		CrashReporter.SetBreadcrumb("luauInit", "sandbox " + script.NetworkPath);
		state.SandboxGlobals();

		// Internal script
		CrashReporter.SetBreadcrumb("luauInit", "internal-ptrs " + script.NetworkPath);
		SetGlobalTablePtr(state, _internalScriptPtr, script);
		SetGlobalTablePtr(state, _loggerPtr, script.Root.ScriptService.Logger);

		CrashReporter.SetBreadcrumb("luauInit", "register-builtins " + script.NetworkPath);
		...
		// Randomize on start
		CrashReporter.SetBreadcrumb("luauInit", "randomseed " + script.NetworkPath);
		GD.Randomize();
		...
		CrashReporter.SetBreadcrumb("luauInit", "lua-libraries " + script.NetworkPath);
		foreach ((string name, Type lib) in LuaLibraries)
		{
			CrashReporter.SetBreadcrumb("luauInit", "lua-library " + name + " " + script.NetworkPath);
			PushCSClass(state, lib);
			state.SetGlobal(name);
		}

		CrashReporter.SetBreadcrumb("luauInit", "global-data " + script.NetworkPath);
		foreach ((string name, Type lib) in ScriptService.GlobalDataMap)
		{
			CrashReporter.SetBreadcrumb("luauInit", "global-data " + name + " " + script.NetworkPath);
			PushCSClass(state, lib);
			state.SetGlobal(name);
		}
		...
		CrashReporter.SetBreadcrumb("luauInit", "assembly-types " + script.NetworkPath);
...
#pragma warning restore IL2026 // Members annotated with 'RequiresUnreferencedCodeAttribute' require dynamic access otherwise can break functionality when trimming application code
		...
		foreach (var t in types)
		{
			...
			instantiableCount++;
			CrashReporter.SetBreadcrumb("luauInit", "instantiable " + instantiableCount + " " + t.Name + " " + script.NetworkPath);
			PushCSClass(state, t);
			state.SetGlobal(t.Name);
		}

		CrashReporter.SetBreadcrumb("luauInit", "static-objects " + script.NetworkPath);
		...
		{
			CrashReporter.SetBreadcrumb("luauInit", "static-object " + key + " " + script.NetworkPath);
			PushValueToLua(state, instance);
			state.SetGlobal(key);
		}

		CrashReporter.SetBreadcrumb("luauInit", "script-game-globals " + script.NetworkPath);
		CrashReporter.SetBreadcrumb("luauInit", "script-global " + script.NetworkPath);
		PushValueToLua(state, script);
 		state.SetGlobal("script");

		CrashReporter.SetBreadcrumb("luauInit", "game-global " + script.NetworkPath);
		PushValueToLua(state, script.Root);
		state.SetGlobal("game");

		CrashReporter.SetBreadcrumb("luauInit", "world-global " + script.NetworkPath);
		PushValueToLua(state, script.Root);
		state.SetGlobal("world");

		CrashReporter.SetBreadcrumb("luauInit", "instance-global " + script.NetworkPath);
		...
		CrashReporter.SetBreadcrumb("luauInit", "enums " + script.NetworkPath);
		if (mobileLuaSafeMode)
		{
			CrashReporter.SetBreadcrumb("luauInit", "enums-mobile-bridge " + script.NetworkPath);
		}
		foreach ((string name, Type e) in ScriptService.EnumMap)
		{
			CrashReporter.SetBreadcrumb("luauInit", "enum " + name + " " + script.NetworkPath);
		...
		if (script.Compatibility)
		{
			CrashReporter.SetBreadcrumb("luauInit", "compat-enums " + script.NetworkPath);
		...
		CrashReporter.SetBreadcrumb("luauInit", "new-main-thread " + script.NetworkPath);
		LuaState mainThread;
		if (Globals.IsMobileBuild || OS.HasFeature("mobile-ui"))
		{
			CrashReporter.SetBreadcrumb("luauInit", "reuse-script-thread-mobile " + script.NetworkPath);
			mainThread = state;
		}
		else
		{
			mainThread = NewThread(state);
		}
		script.LuauMainThread = mainThread;

		CrashReporter.SetBreadcrumb("luauInit", "done " + script.NetworkPath);
		return mainThread;
	}

	private LuaState CreateMobileScriptState()
	{
		LuaState state = new();
		InitializeCache(state);
		state.OpenLibs();
	...
	private void PushMobileInstanceClassProxy(LuaState state, Script script)
	{
		state.NewTable();
	}

	private void PushMobileInstanceSnapshot(LuaState state, Instance? instance, int depth)
	{
		PushMobileInstanceSnapshot(state, instance, depth, []);
	}

	private void PushMobileInstanceSnapshot(LuaState state, Instance? instance, int depth, HashSet<Instance> visited)
	{
		if (instance == null)
		{
			state.PushNil();
		...
		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"new-table {instance.NetworkPath} depth={depth}");
		state.NewTable();

		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"name {instance.NetworkPath}");
		state.PushString(instance.Name ?? "");
		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"name-pushed {instance.NetworkPath}");
		state.SetField(-2, "Name");
		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"name-set {instance.NetworkPath}");

		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"class {instance.NetworkPath}");
		state.PushString(instance.ClassName ?? "");
		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"class-pushed {instance.NetworkPath}");
		state.SetField(-2, "ClassName");
		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"class-set {instance.NetworkPath}");

		if (instance is UILabel label)
		{
			CrashReporter.SetBreadcrumb("mobileLuaProxy", $"text {instance.NetworkPath}");
			state.PushString(label.Text ?? "");
			state.SetField(-2, "Text");
		}

		if (instance is UIField field)
		{
			CrashReporter.SetBreadcrumb("mobileLuaProxy", $"visible {instance.NetworkPath}");
			state.PushBoolean(field.Visible);
			state.SetField(-2, "Visible");
		}

		if (depth <= 0 || !visited.Add(instance))
		{
			CrashReporter.SetBreadcrumb("mobileLuaProxy", $"leaf {instance.NetworkPath}");
			return;
		}

		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"parent {instance.NetworkPath}");
		PushMobileInstanceSnapshot(state, instance.Parent, depth - 1, visited);
		state.SetField(-2, "Parent");

		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"children {instance.NetworkPath}");
		foreach (Instance child in instance.GetChildren())
		{
			if (string.IsNullOrWhiteSpace(child.Name)) continue;
			CrashReporter.SetBreadcrumb("mobileLuaProxy", $"child {instance.NetworkPath}.{child.Name}");
			PushMobileInstanceSnapshot(state, child, depth - 1, visited);
			state.SetField(-2, child.Name);
		}

		CrashReporter.SetBreadcrumb("mobileLuaProxy", $"done {instance.NetworkPath}");
