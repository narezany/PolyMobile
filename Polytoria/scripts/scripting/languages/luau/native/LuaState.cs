...
public partial class LuaState : IDisposable
{
	...
	public static byte[] Compile(string sourceCode)
	{
		...
		*/
		...
		try
		{
			...
			bytecodePtr = NativeBindings.luau_compile_ptr(sourcePtr, (UIntPtr)sourceBytes.Length, IntPtr.Zero, out UIntPtr outSize);
			if (bytecodePtr == IntPtr.Zero)
			{
				//Marshal.FreeHGlobal(optionsPtr);
				throw new Exception("Error compiling Lua source code");
			}
			byte[] bytecode = new byte[(int)outSize];
	...
	{
		CrashReporter.SetBreadcrumb("luaLoad", $"enter nameLen={name?.Length ?? 0} bytecodeLen={compiled?.Length ?? 0}");
		if (Loaded)
		...
		try
		{
			CrashReporter.SetBreadcrumb("luaLoad", "alloc-bytecode");
			bytecodePtr = Marshal.AllocHGlobal(compiled.Length);
			if (compiled.Length > 0)
			{
				CrashReporter.SetBreadcrumb("luaLoad", "copy-bytecode");
				Marshal.Copy(compiled, 0, bytecodePtr, compiled.Length);
			}

			int loadResult = 0;
			CrashReporter.SetBreadcrumb("luaLoad", "name-pointer");
			WithUtf8Pointer(name, namePtr =>
			{
				CrashReporter.SetBreadcrumb("luaLoad", $"native-call-start bytecodeLen={compiled.Length}");
				lock (_lock)
					loadResult = NativeBindings.luau_load_size_ptr(_state, namePtr, bytecodePtr, (UIntPtr)compiled.Length, 0);
				CrashReporter.SetBreadcrumb("luaLoad", "native-call-done result=" + loadResult);
			});
	...
	public LuaStatus LoadString(string name, string source)
	{
		CrashReporter.SetBreadcrumb("luaLoad", $"loadstring-enter nameLen={name?.Length ?? 0} sourceLen={source?.Length ?? 0}");
		if (Loaded)
			throw new Exception("Cannot load when already loaded! Please dispose this VM and create a new one");

		int loadResult = 0;
		WithUtf8Pointer(source, sourcePtr =>
		{
			CrashReporter.SetBreadcrumb("luaLoad", "loadstring-native-start");
			lock (_lock)
				loadResult = NativeBindings.luaL_loadstring_ptr(_state, sourcePtr);
			CrashReporter.SetBreadcrumb("luaLoad", "loadstring-native-done result=" + loadResult);
		});

		if (loadResult != 0)
		{
			throw new LuaException($"Error loading source {name}: " + ToString(-1));
		}
	...
	public void PushString(string s)
	{
		...
		try
		{
			...
			lock (_lock)
				NativeBindings.lua_pushlstring_ptr(_state, unmanaged, (UIntPtr)bytes.Length);
		}
		finally
		{
	...
	{
		WithUtf8Pointer(key, ptr =>
		{
			lock (_lock)
				NativeBindings.lua_setfield_ptr(_state, index, ptr);
		});
	}
	...
	public void GetField(int index, string key)
	{
		...
		{
			lock (_lock)
				NativeBindings.lua_getfield_ptr(_state, index, ptr);
		});
	}

	private static void WithUtf8Pointer(string? value, Action<IntPtr> action)
	{
		byte[] bytes = Encoding.UTF8.GetBytes(value ?? "");
		IntPtr unmanaged = Marshal.AllocHGlobal(bytes.Length + 1);
		try
		{
			if (bytes.Length > 0)
			{
