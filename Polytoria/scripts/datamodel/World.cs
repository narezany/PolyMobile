...
public sealed partial class World : Instance
{
	...
	internal async void DispatchClientScriptRunMobileSafe()
	{
		if (!MobileDevSettings.ClientScriptsEnabled)
		{
			PT.Print("Mobile client scripts disabled");
			CrashReporter.SetBreadcrumb("clientScript", "mobile-disabled");
			return;
		}

		PT.Print("Dispatch Client run mobile safe");
		ClientScript[] scripts = GetDescendants().OfType<ClientScript>().ToArray();
		CrashReporter.SetBreadcrumb("clientScript", $"safe-start count={scripts.Length}");

		for (int i = 0; i < scripts.Length; i++)
		{
			ClientScript script = scripts[i];
			if (script.IsDeleted || script.Ran || script.IsHidden || !script.IsEnabled)
			{
				continue;
			}

			CrashReporter.SetBreadcrumb("clientScript", $"{i + 1}/{scripts.Length} {script.NetworkPath}");
			try
			{
				script.TryRun();
			}
			...
			{
				CrashReporter.Report("World.DispatchClientScriptRunMobileSafe " + script.NetworkPath, ex);
			}

			await Globals.Singleton.WaitFrame();
		}

		CrashReporter.SetBreadcrumb("clientScript", "safe-done");
	}
