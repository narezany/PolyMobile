...
public sealed partial class NetworkService : Instance
{
	...
	public bool IsOfflineLocalSession = false;
	...
	public void CreateOfflineLocalPlayer(int userId = 1144, string username = "Player")
	{
		SetupNetwork();
		InitNodes();
		Root.ListenToNetworkService();

		IsOfflineLocalSession = true;
		LocalPeerID = 1;
		IsServer = true;
		...
		ActivePeerIDs.Remove(1);
		ActivePeerIDs.Add(1);

		EnsureOfflineTestBaseplate();

		Player plr = Globals.LoadInstance<Player>(Root)!;
		_players.PeerIDToPlayer.TryAdd(1, plr);
		...
		plr.IsCreator = true;
		plr.CanChat = true;
		plr.UserPlatform = Globals.IsMobileBuild ? ClientPlatformEnum.Mobile : ClientPlatformEnum.Desktop;
		plr.ChatColor = Root.PlayerDefaults.ChatColor;
		plr.SetNetworkAuthority(1, true);
		plr.NetTransformAuthority = 1;
		...
		Root.PlayerDefaults.AutoLoadAppearance = false;
	...
	private void EnsureOfflineTestBaseplate()
	{
		if (Root.Environment.FindChild<Part>("OfflineTestBaseplate") != null)
		{
			return;
		}

		Part baseplate = Globals.LoadInstance<Part>(Root)!;
		baseplate.Name = "OfflineTestBaseplate";
		baseplate.Anchored = true;
		baseplate.CanCollide = true;
		baseplate.Shape = Part.ShapeEnum.Brick;
		baseplate.Material = Part.PartMaterialEnum.Grass;
		baseplate.Color = new Color(0.32f, 0.83f, 0.36f);
		baseplate.Position = new Vector3(0, -5, 0);
		baseplate.Size = new Vector3(512, 10, 512);
		baseplate.Parent = Root.Environment;
	}

	public async void CreateClient(string address, int port = 24221)
	{
		CrashReporter.SetBreadcrumb("networkService", "create-client-setup");
		SetupNetwork();
		SetProcess(true);

		CrashReporter.SetBreadcrumb("networkService", "create-client-new-instance");
		NetInstance = new();
		...
		CrashReporter.SetBreadcrumb("networkService", "create-client-before-enet");
		await NetInstance.CreateClient(address, port);
		CrashReporter.SetBreadcrumb("networkService", "create-client-after-enet");

		SetupPeer();
		CrashReporter.SetBreadcrumb("networkService", "create-client-setup-peer");
		OnSessionStarted();
		CrashReporter.SetBreadcrumb("networkService", "create-client-session-started");
	}
	...
	{
		CrashReporter.SetBreadcrumb("syncFlow", "world-start");
		if (IsPlaceReplicationDone)
		{
			return;
		}
		...
		CrashReporter.SetBreadcrumb("syncFlow", "world-initnodes");
		InitNodes();

		// Re-run Setup() to restore client-only services (e.g. PlayerGUI) that
		// are not present in the server-replicated hierarchy.
		CrashReporter.SetBreadcrumb("syncFlow", "world-re-setup");
		Root.Setup();

		PT.Print("[Client] World Replicated");
		CrashReporter.SetBreadcrumb("syncFlow", "world-rpc-transform");
		RpcId(1, nameof(NetReqAllTransform));
		CrashReporter.SetBreadcrumb("syncFlow", "world-done");
	}
	...
	{
		CrashReporter.SetBreadcrumb("syncFlow", "transform-start");
		...
		if (_players != null)
		{
			CrashReporter.SetBreadcrumb("syncFlow", "transform-rpc-scripts");
			RpcId(1, nameof(NetReqAllScripts));
			CrashReporter.SetBreadcrumb("syncFlow", "transform-done");
		}
		else
		{
	...
	{
		CrashReporter.SetBreadcrumb("syncFlow", "script-start");
		...
		CrashReporter.SetBreadcrumb("syncFlow", "script-client-world-ready");
		ClientWorldReady?.Invoke();
		...
		if (_players != null)
		{
			CrashReporter.SetBreadcrumb("syncFlow", "script-req-local-player");
			_players.ReqLocalPlayer();
			CrashReporter.SetBreadcrumb("syncFlow", "script-done");
		}
		else
		{
	...
	{
		CrashReporter.SetBreadcrumb("syncFlow", "report-ready-start");
		ClientReady?.Invoke();
		...
		try
		{
			...
			{
				CrashReporter.SetBreadcrumb("syncFlow", "dispatch-client-scripts-mobile-safe");
				Root.DispatchClientScriptRunMobileSafe();
			}
			else
			{
				CrashReporter.SetBreadcrumb("syncFlow", "dispatch-client-scripts");
				Root.DispatchClientScriptRun();
			}
		}
		catch (Exception ex)
		...
		CrashReporter.SetBreadcrumb("syncFlow", "report-ready-done");
	}
