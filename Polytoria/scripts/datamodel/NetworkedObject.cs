...
public partial class NetworkedObject : IScriptObject
{
	...
	internal void RecvReplicate(NetReplicateData data)
	{
		...
		CrashReporter.SetBreadcrumb("recvReplicate", $"start {className} {data.NodePath} props={props.Length}");
		...
		if (existingObj != null)
		{
			...
			CrashReporter.SetBreadcrumb("recvReplicate", $"existing-apply {className} {data.NodePath}");
			netObj.ApplyNetProps(props, data.IsSyncOnce);
			CrashReporter.SetBreadcrumb("recvReplicate", $"existing-done {className} {data.NodePath}");
			return;
		}

		CrashReporter.SetBreadcrumb("recvReplicate", $"load-start {className} {data.NodePath}");
		NetworkedObject? netobj = Globals.LoadNetworkedObject(className);
		CrashReporter.SetBreadcrumb("recvReplicate", $"load-done {className} {data.NodePath}");

		if (netobj == null)
		{
			CrashReporter.SetBreadcrumb("recvReplicate", $"unknown {className} {data.NodePath}");
		...
		CrashReporter.SetBreadcrumb("recvReplicate", $"apply-start {className} {data.NodePath}");
		netobj.ApplyNetProps(props, data.IsSyncOnce);
		CrashReporter.SetBreadcrumb("recvReplicate", $"done {className} {data.NodePath}");
	}
