...
public partial class Instance : NetworkedObject
{
	...
	public async Task<Instance?> WaitChild(string name, float? timeoutSec = null)
	{
		...
		CancellationTokenSource warningCts = new();
	...
	private static void TryCancelWarning(CancellationTokenSource cts)
	{
		try
		{
			cts.Cancel();
		...
		{
			// A detached warning task may race shutdown on mobile.
		}
	}
