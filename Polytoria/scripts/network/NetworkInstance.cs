...
public class NetworkInstance
{
	...
	{
		CrashReporter.SetBreadcrumb("enet", "create-host-start");
		Error e = _peer.CreateHost(DefaultCapacity, maxChannels);
		CrashReporter.SetBreadcrumb("enet", "create-host=" + e);
		_peer.BandwidthLimit(BandwidthInLimit, BandwidthOutLimit);
		CrashReporter.SetBreadcrumb("enet", "bandwidth-ok");
		_peer.Compress(CompressionMode);
		CrashReporter.SetBreadcrumb("enet", "compress-ok");
		...
		CrashReporter.SetBreadcrumb("enet", $"connect-start {address}:{port}");
		_peer.ConnectToHost(address, port);
		CrashReporter.SetBreadcrumb("enet", "connect-called");

		PostPeerCreate();
	}

	public void PollMainThread()
	{
		if (!_manualPump) return;
		...
		{
			CrashReporter.Report("NetworkInstance.PollMainThread", ex);
	...
	private void PostPeerCreate()
	{
		...
		{
			_manualPump = true;
			CrashReporter.SetBreadcrumb("enet", "manual-pump");
			return;
		}

		CrashReporter.SetBreadcrumb("enet", "thread-pump");
